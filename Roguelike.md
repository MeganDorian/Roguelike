...
Карта в системе может быть сгенерирована или считана с файла. За хранение карты в памяти отвечает класс _Map_,
представленный на диаграмме выше.

Для карты, записанной в файл, выдвигаются следующие требования:

- Стенки обозначаются символом '@';
- Персонаж обозначается символом '+';
- Моб обозначается символом '#';
- Предмет обозначается символом '?';
- Пустое пространство, вход и выход обозначаются пробельным символом.

Для карты, записанной в файл, выдвигаются следующие ограничения:

- Вся область карты должна иметь границу в виде стенки;
- У карты должен быть один вход и один выход;
- Вход на карту всегда слева;
- Выход с карты всегда справа;
- На уровне всегда должен быть один и только один персонаж;
- Всегда должен существовать от входа на уровень до выхода с уровня;
- Нельзя задавать характеристики мобам и предметом, так как они должны генерироваться в соответствии с уровнем карты
  для баланса системы.

#### Генерация карты

Всё пространство карты делится на "комнаты" - область, заданная параметрами, передающимися в генератор карты. xRoom -
количество комнат по горизонтали, yRoom - количество комнат по вертикали, size - длина комнаты. Длина и ширина комнаты
совпадает, стены в подсчёт длина не входят. Комнаты разделяются друг с другом одинарной стеной.

Исходя из вышеперечисленного, размер карты определяется по формуле:

- Длина по горизонтали: xRoom * size + xRoom + 1;
- Длина по вертикали: yRoom * size + yRoom + 1.

При генерации осуществляется подбор параметров карты (xRoom, yRoom, size) при помощи рекурсивного алгоритма наибольшего
общего делителя (gcd), так как все параметры карты должны быть целочисленными. При неуспешном нахождении gcd,
осуществляется оптимальный подбор параметров для карты меньшего размера, а до требуемого размера карта дополняется
повторением предпоследнего столбца/строчки, чтобы гарантировать, что не возникнет ситуации, когда нет пути от входа до
выхода с уровня.

Далее выбирается рандомная комната для старта алгоритма генерации и помечается, что в неё есть проход, и пока не
опустеет список комнат:

1. Выбираем рандомную комнату из списка;
2. Генерируем индексы комнаты по горизонтали (x) и по вертикали (y);
3. Если комнаты соединенны между собой, то удаляем номер комнаты из пункта 1 и переходим в пункт 1, иначе переходим к
   пункту 4;
4. Расставляем в случайном порядке "направления" без повторения (существуют следующие направления верх (UP), низ(DOWN),
5. право(RIGHT), лево(LEFT), старт(START), пустота(EMPTY));
6. Проходим по всему списку направлений;
   5.1. Если UP, то проверяем соседнюю комнату сверху ([x][y - 1]). Если в эту комнату есть проход, то помечаем, что в
   комнату [x][y] тоже есть проход, и помечаем, что он находится сверху.
   5.2. Если DOWN, то проверяем соседнюю комнату снизу ([x][y + 1]). Если в эту комнату есть проход, то помечаем, что в
   комнату [x][y] тоже есть проход, и помечаем, что он находится снизу.
   5.3. Если LEFT, то проверяем соседнюю комнату слева ([x - 1][y]). Если в эту комнату есть проход, то помечаем, что в
   комнату [x][y] тоже есть проход, и помечаем, что он находится слева.
   5.4. Если RIGHT, то проверяем соседнюю комнату справа ([x + 1][y]). Если в эту комнату есть проход, то помечаем, что
   в комнату [x][y] тоже есть проход, и помечаем, что он находится справа.

Таким образом гарантируется, что на карте можно пройти в любую комнату, следовательно, всегда будет путь от входа на
уровень до выхода с уровня.

После выполнения вышеописанных шагов, проходим по каждой комнате и "стираем" (заполняем пробельными символами) стены с
запомненных направлений (может оказаться, что у комнаты стёрты все стены, это считается валидным случаем).
