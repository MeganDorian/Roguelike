...
3. Если комнаты соединенны между собой, то удаляем номер комнаты из пункта 1 и переходим в пункт 1, иначе переходим к
   пункту 4;
4. Расставляем в случайном порядке "направления" без повторения (существуют следующие направления верх (UP), низ(DOWN), право(RIGHT), лево(LEFT), старт(START), пустота(EMPTY));
5. Проходим по всему списку направлений;
...
После выполнения вышеописанных шагов, проходим по каждой комнате и "стираем" (заполняем пробельными символами) стены с запомненных направлений (может оказаться, что у комнаты стёрты все стены, это считается валидным случаем).
