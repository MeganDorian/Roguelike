...
public class PlayerPrinter {
    ...
    private final TerminalSize playerBlockSize;
    ...
    private void printPlayerStats(Game game, int column, int startRow) {
        textGraphics.fillRectangle(new TerminalPosition(column, startRow), playerBlockSize, SpecialCharacters.SPACE);
        List<String> playerInfo = game.getPlayer().getInfo();
        textGraphics.putString(column + 1, startRow, "DUNGEON LEVEL: " + game.getDungeonLevel());
        int row = 2;
        
        textGraphics.drawRectangle(new TerminalPosition(column, row), playerBlockSize, SpecialCharacters.DELIMITER);
